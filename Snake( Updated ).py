...
class MAIN:
    ...
    def draw_game_over_message(self):
        ...
        screen.blit(game_over_text, (cell_size * 4, cell_size * 6))
        screen.blit(restart_text, (cell_size * 3.5, cell_size * 8))
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_UP and not main_game.game_over and not main_game.greedy_mode:
                if main_game.snake.direction.y != 1:
                    main_game.snake.direction = Vector2(0, -1)
            if event.key == pygame.K_DOWN and not main_game.game_over and not main_game.greedy_mode:
                if main_game.snake.direction.y != -1:
                    main_game.snake.direction = Vector2(0, 1)
            if event.key == pygame.K_LEFT and not main_game.game_over and not main_game.greedy_mode:
                if main_game.snake.direction.x != 1:
                    main_game.snake.direction = Vector2(-1, 0)
            if event.key == pygame.K_RIGHT and not main_game.game_over and not main_game.greedy_mode:
                if main_game.snake.direction.x != -1:
                    main_game.snake.direction = Vector2(1, 0)
            if event.key == pygame.K_r and main_game.game_over:  # Restart the game
                main_game = MAIN()  # Re-initialize the game
            if event.key == pygame.K_a:  # Switch to greedy search mode
                main_game.greedy_mode = not main_game.greedy_mode
